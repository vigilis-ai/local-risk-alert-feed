/**
 * Test Atlanta plugins for a specific location.
 *
 * Usage: npx tsx scripts/test-atlanta.ts [lat] [lng] [radius]
 * Default: Atlanta downtown (33.7490, -84.3880) with 10km radius
 *
 * Tip: to focus on the Delta Air Lines campus / Hartsfield-Jackson airport,
 *   npx tsx scripts/test-atlanta.ts 33.6407 -84.4277 8000
 *
 * The Georgia 511 traffic plugin requires GEORGIA_511_API_KEY; it is only
 * registered when that variable is set.
 */

import { AlertFeed } from '../src';
import { NWSWeatherPlugin } from '../src/plugins/weather';
import { AtlantaCrimePlugin, AtlantaTrafficPlugin } from '../src/plugins/atlanta';

async function testAtlanta() {
  const args = process.argv.slice(2);
  const latitude = parseFloat(args[0]) || 33.749;
  const longitude = parseFloat(args[1]) || -84.388;
  const radiusMeters = parseInt(args[2], 10) || 10000;

  console.log('='.repeat(70));
  console.log('ATLANTA PLUGIN TEST');
  console.log('='.repeat(70));
  ...
  const registrations: Array<{ plugin: any }> = [
    { plugin: new NWSWeatherPlugin() },
    { plugin: new AtlantaCrimePlugin() },
  ];

  if (process.env.GEORGIA_511_API_KEY) {
    registrations.push({ plugin: new AtlantaTrafficPlugin() });
  } else {
    console.log('[note] GEORGIA_511_API_KEY not set — skipping AtlantaTrafficPlugin\n');
  }

  await feed.registerPlugins(registrations);
  ...
  // Query past-7d to now+1d (combines historical and current)
  console.log('\n' + '-'.repeat(70));
...
testAtlanta().catch(console.error);
